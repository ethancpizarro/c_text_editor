...
void editorUpdateSyntax(e_row * row)
{
    ...
    if (changed && row -> idx + 1 < E.num_rows) editorUpdateSyntax(&E.row[row -> idx + 1]);
}
    ...
    for (unsigned int j = 0; j < HLDB_ENTRIES; ++j)
    {
        ...
        while (s -> file_match[i])
        {
            ...
            if ((is_ext && ext && !strcmp(ext, s -> file_match[i])) || (!is_ext && strstr(E.file_name, s -> file_match[i])))
            {
                ...
                int file_row;
                for (file_row = 0; file_row < E.num_rows; ++file_row) editorUpdateSyntax(&E.row[file_row]);
                return;
            }
            ++i;
        }
...
char * editorRowsToString(int * buf_len)
{
    int tot_len = 0;
    int j;
    for (j = 0; j < E.num_rows; ++j) tot_len += E.row[j].size + 1;
    *buf_len = tot_len;
